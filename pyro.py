...
class CoreUI(object):
    ...
    def __init__(self, lexer, filename="Untitled", filepath=None, mod=None, settings={}):
        ...
        pywinstyles.apply_style(self.root, "dark" if self.theme in ["Dark", "Midnight"] else "normal")
        ...
        # Files
        self.root.bind("<Control-KeyPress-n>", partial(MenuMethods.new_file, self))
        self.root.bind("<Control-KeyPress-N>", partial(MenuMethods.new_file, self))
        ...
        self.keywords = load_json("resources/keywords.json")

        # Define snippets
        self.snippets = load_json("resources/snippets.json")
    ...
    def control_backspace(self, e):
        cursor_pos = self.text.index("insert")

        # Get all characters before cursor
        content_before_cursor = self.text.get("1.0", cursor_pos)

        if content_before_cursor.endswith(' '):
            # Delete all spaces until the first non-space character
            while content_before_cursor.endswith(' '):
                self.text.delete(f"{cursor_pos} -1c", cursor_pos)
                cursor_pos = self.text.index("insert")  # Update cursor position
                content_before_cursor = self.text.get("1.0", cursor_pos)
            return "break"  # Prevent default behavior
        else:
            # Only delete a word if we're not at the beginning
            if cursor_pos != "1.0":  # Ensure we are not at the very start
                try:
                    word_start = self.text.index(f"{cursor_pos} -1 wordstart")
                    self.text.delete(word_start, cursor_pos)
                    return "break"  # Prevent default behavior
                except TclError:
                    # Handle case where there is no word to delete
                    print("No word to delete at this position.")  # You can customize this message
                    return "break"  # Prevent default behavior
            else:
                # If at the start, just return without any deletion
                return "break"  # Prevent default behavior

    def shift_backspace(self, e):
        cursor_pos = self.text.index("insert")

        # Calculate position to delete
        prev_char = self.text.get(f"{cursor_pos} -1c", cursor_pos)

        if prev_char == ' ':
            # Count spaces until the next factor of 4
            pos_in_line = int(cursor_pos.split('.')[1])
            spaces_to_delete = pos_in_line % 4 or 4  # Get the number of spaces to delete
            self.text.delete(f"{cursor_pos} -{spaces_to_delete}c", cursor_pos)  # Delete the spaces
            return "break"  # Prevent default behavior
    ...
    def handle_backspace(self, e):
        ...
        global auto_close_chars
        
        # Get the character before the cursor
        prev_char = self.text.get(f"{current_index} - 1c")
        next_char = self.text.get(f"{current_index}")  # Character after the cursor
        
        # Check if the previous character is an opening character
        if prev_char in auto_close_chars.keys():
            closing_char = auto_close_chars[prev_char]  # Get the corresponding closing character
            print("closing:" + closing_char)
            print("next:" + next_char)
            print("prev:" + prev_char)
            
            if next_char == closing_char:
                self.text.delete(current_index)
                self.text.delete(f"{current_index} - 1c")
                return "break"  # Prevent further processing of the Backspace key
    ...
    def event_key(self, event):
        ...
        if char in auto_close_chars:
            next_char = self.text.get(f"{current_index} + 1c") 

            if next_char in ('', ' '):
                self.text.insert("insert", auto_close_chars[char])
                self.text.mark_set("insert", current_index)
                
        # self.prev_char = self.text.get(f"{current_index} - 1c")
    ...
    #     """Update minimap with the content of the text editor."""
    #     self.minimap.config(state='normal')
    #     self.minimap.delete(1.0, END)
    #     self.minimap.insert(1.0, self.text.get(1.0, END))
